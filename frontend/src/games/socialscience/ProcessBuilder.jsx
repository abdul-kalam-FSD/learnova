...
// Social Science's second mechanic alongside
// SOCIAL_SCIENCE_CIVIC_DECISION (a single-choice scenario pick —
// "which response is fairest?"). This arranges the steps of a real
// civic process into the order they actually happen — same order-
// sensitive family as HISTORY_TIMELINE_BUILDER / CS_CODE_ORDER_
// BUILDER / ENGLISH_SENTENCE_BUILDER (see gameControllers.js's
// shared orderedPieceIds === correct_order check), reusing that
// generic backend logic — no new backend scoring code needed.
const GAME_TYPE = "SOCIAL_SCIENCE_PROCESS_BUILDER";

// ---------- Level select ----------
function LevelSelectScreen({ levels, xp, streak, onBack, onPick }) {
  return (
    <GamePage>
      <GameTopBar label="Civic Process Builder" xp={xp} streak={streak} onBack={onBack} />
      <GamePanel>
        <span className="clue-card__label">SOCIAL SCIENCE · CIVIC PROCESSES</span>
        <h1 className="text-2xl font-bold mt-1 mb-3">Build the Process</h1>
        <p className="hint-text text-sm mb-4">
          Arrange the scrambled steps into the order they actually happen in real life.
        </p>
...
function LobbyScreen({ level, levelIndex, totalLevels, xp, streak, xpInfo, onBack, onStart }) {
  return (
    <GamePage>
      ...
      <GameLobby
        title={level.title}
        subjectLabel="SOCIAL SCIENCE · CIVIC PROCESSES"
      ...
      <GamePanel>
        ...
        {feedback && (
          <GameFeedback
            ...
            whatYouLearned={
              feedback.isCorrect
                ? "Civic processes usually follow a logical sequence — each step depends on the one before it actually being done first."
                : null
            }
          />
        )}
...
function ProcessBuilderGame() {
  ...
  if (stage === "loading") return <GameLoadingState label="Loading Civic Process Builder..." />;
  if (stage === "error")
    return (
