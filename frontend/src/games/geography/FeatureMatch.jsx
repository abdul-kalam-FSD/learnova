...
// Geography's second mechanic alongside GEOGRAPHY_ROUTE_BUILDER
// (which ranks/sequences stops or facts in order). This matches a
// geographic fact, feature or process to its correct description —
// same mapping-equality family as HISTORY_CAUSE_EFFECT_MATCH /
// CHEMISTRY_MATCH / sixteen others (see gameControllers.js's shared
// mapping === correct_mapping check), reusing that generic backend
// logic — no new backend scoring code needed. UI cloned from
// history/CauseEffectMatch.jsx.
const GAME_TYPE = "GEOGRAPHY_FEATURE_MATCH";

// ---------- Level select ----------
function LevelSelectScreen({ levels, xp, streak, onBack, onPick }) {
  return (
    <GamePage>
      <GameTopBar label="Feature Match" xp={xp} streak={streak} onBack={onBack} />
      <GamePanel>
        <span className="clue-card__label">GEOGRAPHY · FEATURES & PROCESSES</span>
        <h1 className="text-2xl font-bold mt-1 mb-3">Match Each Feature to Its Description</h1>
        <p className="hint-text text-sm mb-4">
          Assign the correct description to each geographic feature or process — some
          options sound plausible but are wrong, so read carefully.
        </p>
            ...
            >
              <span className="flex flex-col">
                ...
                <span className="hint-text text-xs">
                  {level.payload.slots.length} features
                </span>
              </span>
...
function LobbyScreen({ level, levelIndex, totalLevels, xp, streak, xpInfo, onBack, onStart }) {
  return (
    <GamePage>
      ...
      <GameLobby
        title={level.title}
        subjectLabel="GEOGRAPHY · FEATURES & PROCESSES"
...
function MatchScreen({ level, sessionId, xp, streak, onBack, onSolved, levelIndex, totalLevels }) {
  ...
  return (
    <GamePage>
      ...
      />
      <GamePanel>
        <GameObjective>Match each geographic feature or process to its correct description.</GameObjective>

        <span className="clue-card__label">SCENARIO</span>
        <h2 className="text-lg font-bold mt-1 mb-4">{scenario}</h2>

        <p className="hint-text text-xs mb-2">Features (pick a description below, then tap a feature):</p>
        <div className="flex flex-col gap-2 mb-5">
          {slots.map((slot) => (
            ...
            >
              ...
              <span className="hint-text text-xs">
                {mapping[slot.id] ? componentById(mapping[slot.id])?.label : "— not assigned yet —"}
              </span>
            </button>
          ))}
        </div>

        <p className="hint-text text-xs mb-2">Descriptions:</p>
        <div className="flex flex-col gap-2 mb-4">
          {tray.map((component) => (
            <button
              ...
              disabled={feedback?.isCorrect}
              className={`dragdrop-item px-3 py-2 rounded-lg text-sm text-left select-none ${
        ...
        {feedback && (
          <GameFeedback
            ...
            verdict={feedback.isCorrect ? "✓ Every feature matched correctly!" : "✕ Not quite — some pairs are wrong."}
            explanation={!feedback.isCorrect ? feedback.hint || hint : null}
            whatYouLearned={
              feedback.isCorrect
                ? "Geographic features and processes each have their own defining characteristics — knowing the difference helps you read maps and landscapes correctly."
                : null
            }
          />
        )}
...
function FeatureMatchGame() {
  ...
  if (stage === "loading") return <GameLoadingState label="Loading Feature Match..." />;
  if (stage === "error")
    return (
...
// FeatureMatch is migrated to the focused game-session mode (Phase 5C-A)
