...
// Commerce's first mechanic (Accountancy / Business Studies /
// Economics, Grades 11-12): matches an accounting, business, or
// economic term to its correct definition — same mapping-equality
// family as HISTORY_CAUSE_EFFECT_MATCH / GEOGRAPHY_FEATURE_MATCH /
// sixteen others (see gameControllers.js's shared mapping ===
// correct_mapping check), reusing that generic backend logic — no
// new backend scoring code needed. UI cloned from
// geography/FeatureMatch.jsx.
const GAME_TYPE = "COMMERCE_CONCEPT_MATCH";
            ...
            >
              <span className="flex flex-col">
                ...
                <span className="hint-text text-xs">
                  {level.payload.slots.length} terms
                </span>
              </span>
  ...
  return (
    <GamePage>
      ...
      <GamePanel>
        ...
        <p className="hint-text text-xs mb-2">Definitions:</p>
...
function ConceptMatchGame() {
  ...
  if (stage === "loading") return <GameLoadingState label="Loading Concept Match..." />;
  if (stage === "error")
    return (
...
// ConceptMatch is migrated to the focused game-session mode (Phase 5C-A)
