...
const GAME_TYPE = "HISTORY_CAUSE_EFFECT_MATCH";

// ---------- Level select ----------
function LevelSelectScreen({ levels, xp, streak, onBack, onPick }) {
  return (
    <GamePage>
      <GameTopBar label="Cause & Effect Match" xp={xp} streak={streak} onBack={onBack} />
      <GamePanel>
        <span className="clue-card__label">HISTORY · CAUSE & EFFECT</span>
        <h1 className="text-2xl font-bold mt-1 mb-3">Match Each Event to Its Effect</h1>
        <p className="hint-text text-sm mb-4">
          Assign the correct outcome to each event — some options are
          plausible but wrong, so read carefully.
        </p>
            ...
            >
              <span className="flex flex-col">
                ...
                <span className="hint-text text-xs">
                  {level.payload.slots.length} events
                </span>
              </span>
...
function LobbyScreen({ level, levelIndex, totalLevels, xp, streak, xpInfo, onBack, onStart }) {
  return (
    <GamePage>
      ...
      <GameLobby
        title={level.title}
        subjectLabel="HISTORY · CAUSE & EFFECT"
...
function MatchScreen({ level, sessionId, xp, streak, onBack, onSolved, levelIndex, totalLevels }) {
  ...
  return (
    <GamePage>
      ...
      />
      <GamePanel>
        <GameObjective>Match each historical cause to its correct effect.</GameObjective>

        <span className="clue-card__label">SCENARIO</span>
        <h2 className="text-lg font-bold mt-1 mb-4">{scenario}</h2>

        <p className="hint-text text-xs mb-2">Events (pick an outcome below, then tap an event):</p>
        ...
        <p className="hint-text text-xs mb-2">Outcomes:</p>
        ...
        {feedback && (
          <GameFeedback
            ...
            verdict={feedback.isCorrect ? "✓ Every event matched correctly!" : "✕ Not quite — some pairs are wrong."}
            explanation={!feedback.isCorrect ? feedback.hint || hint : null}
            whatYouLearned={
              feedback.isCorrect
                ? "Historical events rarely happen in isolation — each cause sets off a specific, traceable effect."
                : null
            }
          />
        )}
...
function CauseEffectMatchGame() {
  ...
  if (stage === "loading") return <GameLoadingState label="Loading Cause & Effect Match..." />;
  if (stage === "error")
    return (
...
// CauseEffectMatch is migrated to the focused game-session mode (Phase 5C-A)
// -- wrapped in GameFrame so it renders like the six Phase 1B/1C
// representative games already did (see GameFrame.jsx).
function CauseEffectMatch() {
  return (
    <GameFrame>
      <CauseEffectMatchGame />
    </GameFrame>
  );
}

export default CauseEffectMatch;
