...
const GAME_TYPE = "CS_DEBUGGING_LAB";
            ...
            >
              <span className="flex flex-col">
                ...
                <span className="hint-text text-xs">
                  {level.payload.code_lines.length}-line algorithm
                </span>
              </span>
...
function DebugScreen({ level, sessionId, xp, streak, onBack, onSolved, levelIndex, totalLevels }) {
  const { scenario_label, code_lines, hint } = level.payload;
  ...
  return (
    <GamePage>
      ...
      <GamePanel>
        ...
        <p className="hint-text text-xs mb-2">Tap the buggy line:</p>
        <div className="flex flex-col gap-1 mb-4 font-mono text-sm">
          {code_lines.map((line, i) => (
            <button
              ...
              className={`dragdrop-item ${lineClass(line.id)} px-3 py-2 rounded-lg text-left select-none`}
            >
              <span className="hint-text mr-2">{i + 1}.</span>
              {line.code_text}
            </button>
          ))}
        </div>
        ...
        {feedback && (
          <GameFeedback
            ...
            verdict={feedback.isCorrect ? "✓ Bug found!" : "✕ That line isn't the bug."}
        ...
        {!feedback?.isCorrect ? (
          <GamePrimaryButton disabled={!picked || submitting} onClick={runDebugger}>
            {submitting ? "Running..." : "Run Debugger"}
          </GamePrimaryButton>
        ) : (
...
function ResultScreen({ level, result, xp, streak, onPlayAnother, onHome, onBackToChapter, onNextGame }) {
  return (
    <GameResults
      ...
      playAgainLabel="Debug Another Algorithm"
      onDashboard={onHome}
...
function DebuggingLabGame() {
  ...
  if (stage === "loading") return <GameLoadingState label="Loading Debugging Lab..." />;
  if (stage === "error")
    return (
