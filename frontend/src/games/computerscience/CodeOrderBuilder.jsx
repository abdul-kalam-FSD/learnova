...
const GAME_TYPE = "CS_CODE_ORDER_BUILDER";

// ---------- Level select ----------
function LevelSelectScreen({ levels, xp, streak, onBack, onPick }) {
  return (
    <GamePage>
      <GameTopBar label="Code Order Builder" xp={xp} streak={streak} onBack={onBack} />
      <GamePanel>
        <span className="clue-card__label">COMPUTER SCIENCE · PYTHON</span>
        <h1 className="text-2xl font-bold mt-1 mb-3">Build the Program</h1>
        <p className="hint-text text-sm mb-4">
          Arrange the scrambled lines into the order they need to run in for
            ...
            >
              <span className="flex flex-col">
                ...
                <span className="hint-text text-xs">
                  {level.payload.scrambled_lines.length}-line program
                </span>
              </span>
...
function LobbyScreen({ level, levelIndex, totalLevels, xp, streak, xpInfo, onBack, onStart }) {
  return (
    <GamePage>
      ...
      <GameLobby
        title={level.title}
        subjectLabel="COMPUTER SCIENCE · PYTHON"
...
function CodeOrderScreen({ level, sessionId, xp, streak, onBack, onSolved, levelIndex, totalLevels }) {
  const { scenario_label, scrambled_lines, hint } = level.payload;
  const [tray, setTray] = useState(scrambled_lines);
  const [placed, setPlaced] = useState([]);
  const [feedback, setFeedback] = useState(null);
  const [submitting, setSubmitting] = useState(false);

  const totalSlots = scrambled_lines.length;

  const placeLine = (line) => {
    if (feedback?.isCorrect) return;
    setTray((prev) => prev.filter((l) => l.id !== line.id));
    setPlaced((prev) => [...prev, line]);
    setFeedback(null);
  };

  // Tap a line already placed to undo back to it (same pattern as
  // Route Builder / Timeline Builder / Equation Builder).
  ...
  const checkOrder = () => {
    ...
    api
      .post(`/games/${sessionId}/attempt`, {
        orderedPieceIds: placed.map((l) => l.id),
      })
  ...
  return (
    <GamePage>
      ...
      />
      <GamePanel>
        <GameObjective>Arrange the code lines in the order the program must run.</GameObjective>
        ...
        <p className="hint-text text-xs mb-2">
          Program so far (tap a line to undo back to it):
        </p>
        <div className="flex flex-col gap-2 mb-5 font-mono">
        ...
        <p className="hint-text text-xs mb-2">Tap lines to add them in order:</p>
        <div className="flex flex-col gap-2 mb-4 font-mono">
          {tray.map((line) => (
            <button
              key={line.id}
              onClick={() => placeLine(line)}
        ...
        {feedback && (
          <GameFeedback
            ...
            verdict={feedback.isCorrect ? "✓ Program runs correctly!" : "✕ That's not the right order."}
            explanation={!feedback.isCorrect ? feedback.hint || hint : null}
            whatYouLearned={
              feedback.isCorrect
                ? "A program executes step by step — the order of instructions determines whether the logic actually works."
                : null
            }
          />
        )}
        ...
        {!feedback?.isCorrect ? (
          <GamePrimaryButton
            ...
            onClick={checkOrder}
          >
            {submitting ? "Checking..." : "Run Program"}
          </GamePrimaryButton>
        ) : (
...
function ResultScreen({ level, result, xp, streak, onPlayAnother, onHome, onBackToChapter, onNextGame }) {
  return (
    <GameResults
      ...
      playAgainLabel="Build Another Program"
      onDashboard={onHome}
...
function CodeOrderBuilderGame() {
  ...
  if (stage === "loading") return <GameLoadingState label="Loading Code Order Builder..." />;
  if (stage === "error")
    return (
...
// CodeOrderBuilder is migrated to the focused game-session mode (Phase 5C-A)
// -- wrapped in GameFrame so it renders like the six Phase 1B/1C
// representative games already did (see GameFrame.jsx).
function CodeOrderBuilder() {
  return (
    <GameFrame>
      <CodeOrderBuilderGame />
    </GameFrame>
  );
}

export default CodeOrderBuilder;
