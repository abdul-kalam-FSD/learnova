...
const GAME_TYPE = "PHYSICS_WORK_ENERGY_POWER_SPEED_CHALLENGE";
...
// Second mechanic for the Grade 11 "Work, Energy and Power" chapter,
// alongside the PHYSICS_MATCH classification challenges in
// seedPhysicsGrade11b.js — that one is the qualitative side (which
// energy concept applies), this is the quantitative side
// (KE = ½mv², W = F×d, P = W/t) that chapter never had a mechanic
// for. Same batch/timer skeleton as Ohm's Law Speed Challenge,
// reusing MULTI_QUESTION_GAME_TYPES / checkMultiQuestionAttempt
// server-side as-is (typed-number questions already supported
// there) — only the theme and question content are new.

// ---------- Round select ----------
function RoundSelectScreen({ levels, xp, streak, onBack, onPick }) {
  return (
    <GamePage>
      <GameTopBar label="Work, Energy & Power Speed Challenge" xp={xp} streak={streak} onBack={onBack} />
      <GamePanel>
        <span className="clue-card__label">PHYSICS · WORK, ENERGY AND POWER</span>
        <h1 className="text-2xl font-bold mt-1 mb-3">Quick-Fire Energy Math</h1>
        <p className="hint-text text-sm mb-4">
          Type the answer and submit before time runs out — KE = ½mv², W = F × d,
          and P = W / t.
        </p>
...
function LobbyScreen({ level, levelIndex, totalLevels, xp, streak, xpInfo, onBack, onStart }) {
  ...
  return (
    <GamePage>
      ...
      <GameLobby
        title={level.title}
        subjectLabel="PHYSICS · WORK, ENERGY AND POWER"
        objective={
...
function RoundScreen({ level, xp, streak, onBack, onRoundScored, levelIndex, totalLevels }) {
  ...
  return (
    <GamePage>
      ...
      />
      <GamePanel>
        <GameObjective>Solve each energy/power question correctly before the timer runs out.</GameObjective>
    ...
    <GamePage>
      ...
      <GamePanel>
        ...
        <GameFeedback
          ...
          whatYouLearned={
            isCorrect
              ? "Fast energy math comes from knowing KE = ½mv², W = F × d, and P = W / t cold, and keeping your units straight."
              : null
          }
        />
...
function ResultScreen({ level, result, scoreResult, xp, streak, onPlayAnother, onHome, onBackToChapter, onNextGame }) {
  return (
    <GameResults
      ...
      badgeText="ENERGY & POWER CHALLENGE COMPLETE"
      title={level.title}
...
function WorkEnergyPowerSpeedChallengeGame() {
  ...
  if (stage === "loading") return <GameLoadingState label="Loading Work, Energy & Power Speed Challenge..." />;
  if (stage === "error")
    return (
...
// WorkEnergyPowerSpeedChallenge is migrated to the focused game-session mode (Phase 5C-A)
// -- wrapped in GameFrame so it renders like the six Phase 1B/1C
// representative games already did (see GameFrame.jsx).
function WorkEnergyPowerSpeedChallenge() {
  return (
    <GameFrame>
      <WorkEnergyPowerSpeedChallengeGame />
    </GameFrame>
  );
}

export default WorkEnergyPowerSpeedChallenge;