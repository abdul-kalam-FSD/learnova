...
const GAME_TYPE = "PHYSICS_CAPACITANCE_SPEED_CHALLENGE";
...
// Second mechanic for the Grade 11 "Electrostatics" chapter, alongside
// the PHYSICS_CIRCUIT_BUILDER matching challenges in
// seedPhysicsGrade11.js — that one is the qualitative side (which
// outcome follows which capacitor scenario), this is the quantitative
// side (C = Q/V, series/parallel combined capacitance) that chapter
// never had a mechanic for. Same batch/timer skeleton as Ohm's Law
// Speed Challenge, reusing MULTI_QUESTION_GAME_TYPES /
// checkMultiQuestionAttempt server-side as-is (typed-number questions
// already supported there) — only the theme and question content are new.

// ---------- Round select ----------
function RoundSelectScreen({ levels, xp, streak, onBack, onPick }) {
  return (
    <GamePage>
      <GameTopBar label="Capacitance Speed Challenge" xp={xp} streak={streak} onBack={onBack} />
      <GamePanel>
        <span className="clue-card__label">PHYSICS · ELECTROSTATICS</span>
        <h1 className="text-2xl font-bold mt-1 mb-3">Quick-Fire Capacitance Math</h1>
        <p className="hint-text text-sm mb-4">
          Type the answer and submit before time runs out — C = Q / V, and
          combined capacitance for series/parallel capacitors.
        </p>
...
function LobbyScreen({ level, levelIndex, totalLevels, xp, streak, xpInfo, onBack, onStart }) {
  ...
  return (
    <GamePage>
      ...
      <GameLobby
        title={level.title}
        subjectLabel="PHYSICS · ELECTROSTATICS"
        objective={
...
function RoundScreen({ level, xp, streak, onBack, onRoundScored, levelIndex, totalLevels }) {
  ...
  return (
    <GamePage>
      ...
      />
      <GamePanel>
        <GameObjective>Solve each capacitance question correctly before the timer runs out.</GameObjective>
    ...
    <GamePage>
      ...
      <GamePanel>
        ...
        <GameFeedback
          ...
          whatYouLearned={
            isCorrect
              ? "Fast capacitance math comes from knowing C = Q / V cold, and remembering series capacitors combine like parallel resistors (and vice versa)."
              : null
          }
        />
...
function ResultScreen({ level, result, scoreResult, xp, streak, onPlayAnother, onHome, onBackToChapter, onNextGame }) {
  return (
    <GameResults
      ...
      badgeText="CAPACITANCE CHALLENGE COMPLETE"
      title={level.title}
...
function CapacitanceSpeedChallengeGame() {
  ...
  if (stage === "loading") return <GameLoadingState label="Loading Capacitance Speed Challenge..." />;
  if (stage === "error")
    return (
...
// CapacitanceSpeedChallenge is migrated to the focused game-session mode (Phase 5C-A)
// -- wrapped in GameFrame so it renders like the six Phase 1B/1C
// representative games already did (see GameFrame.jsx).
function CapacitanceSpeedChallenge() {
  return (
    <GameFrame>
      <CapacitanceSpeedChallengeGame />
    </GameFrame>
  );
}

export default CapacitanceSpeedChallenge;