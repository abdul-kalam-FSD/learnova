...
const GAME_TYPE = "PHYSICS_MATCH";

// ---------- Level select ----------
function LevelSelectScreen({ levels, xp, streak, onBack, onPick }) {
  return (
    <GamePage>
      <GameTopBar label="Magnetism Match" xp={xp} streak={streak} onBack={onBack} />
      <GamePanel>
        <span className="clue-card__label">PHYSICS · MAGNETS</span>
        <h1 className="text-2xl font-bold mt-1 mb-3">Match Magnetic or Non-Magnetic</h1>
        <p className="hint-text text-sm mb-4">
          Match each object or effect to the category it belongs to — extra
          cards in the tray are decoys, so double-check before you assign.
        </p>
            ...
            >
              <span className="flex flex-col">
                ...
                <span className="hint-text text-xs">
                  {level.payload.slots.length} items
                </span>
              </span>
...
function MatchScreen({ level, sessionId, xp, streak, onBack, onSolved, levelIndex, totalLevels }) {
  ...
  return (
    <GamePage>
      ...
      />
      <GamePanel>
        <GameObjective>Match each object or effect to whether it's magnetic or non-magnetic.</GameObjective>
        ...
        <p className="hint-text text-xs mb-2">
          Objects & effects (pick a category below, then tap a card):
        </p>
        ...
        <p className="hint-text text-xs mb-2">Categories:</p>
        ...
        {feedback && (
          <GameFeedback
            ...
            whatYouLearned={
              feedback.isCorrect
                ? "Only a few metals — like iron, nickel, and cobalt — are magnetic; most everyday materials aren't."
                : null
            }
          />
        )}
...
function ResultScreen({ level, result, xp, streak, onPlayAnother, onHome, onBackToChapter, onNextGame }) {
  return (
    <GameResults
      ...
      playAgainLabel="Match More Objects"
      onDashboard={onHome}
...
function PhysicsMatchGame() {
  ...
  if (stage === "loading") return <GameLoadingState label="Loading Magnetism Match..." />;
  if (stage === "error")
    return (
