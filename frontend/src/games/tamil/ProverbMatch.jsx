...
const GAME_TYPE = "TAMIL_PROVERB_MATCH";

// ---------- Level select ----------
function LevelSelectScreen({ levels, xp, streak, onBack, onPick }) {
  return (
    <GamePage>
      <GameTopBar label="Proverb Match" xp={xp} streak={streak} onBack={onBack} />
      <GamePanel>
        <span className="clue-card__label">தமிழ் · பழமொழிகள்</span>
        <h1 className="text-2xl font-bold mt-1 mb-3">பழமொழி பொருத்துக</h1>
        <p className="hint-text text-sm mb-4">
          ஒவ்வொரு பழமொழிக்கும் அதன் சரியான பொருளைப் பொருத்துக — ஏமாற்றும்
          பதில்களும் இருக்கும், கவனமாக இரு.
        </p>
            ...
            >
              <span className="flex flex-col">
                ...
                <span className="hint-text text-xs">
                  {level.payload.slots.length} பழமொழிகள்
                </span>
              </span>
...
function MatchScreen({ level, sessionId, xp, streak, onBack, onSolved, levelIndex, totalLevels }) {
  ...
  return (
    <GamePage>
      ...
      />
      <GamePanel>
        <GameObjective>ஒவ்வொரு பழமொழியையும் அதன் சரியான பொருளுடன் பொருத்துக.</GameObjective>
        ...
        <p className="hint-text text-xs mb-2">
          பழமொழிகள் (கீழே ஒரு பொருளைத் தேர்ந்தெடுத்து, பின் ஒரு பழமொழியில்
          தட்டவும்):
        </p>
        <div className="flex flex-col gap-2 mb-5">
          {slots.map((slot) => (
            ...
            >
              ...
              <span className="hint-text text-xs">
                {mapping[slot.id] ? componentById(mapping[slot.id])?.label : "— இன்னும் தேர்ந்தெடுக்கவில்லை —"}
              </span>
            </button>
          ))}
        </div>

        <p className="hint-text text-xs mb-2">பொருள்கள்:</p>
        ...
        {feedback && (
          <GameFeedback
            ...
            verdict={feedback.isCorrect ? "✓ அனைத்தும் சரியாக பொருந்தியது!" : "✕ எல்லாம் சரியில்லை."}
            explanation={!feedback.isCorrect ? feedback.hint || hint : null}
            whatYouLearned={
              feedback.isCorrect
                ? "பழமொழிகள் ஒவ்வொன்றும் ஒரு குறிப்பிட்ட வாழ்க்கை படிப்பினையை சுருக்கமாக தெரிவிக்கின்றன."
                : null
            }
          />
        )}
        ...
        {!feedback?.isCorrect ? (
          <GamePrimaryButton disabled={!allSlotsFilled || submitting} onClick={checkProverbs}>
            {submitting ? "சரிபார்க்கிறது..." : "சரிபார்"}
          </GamePrimaryButton>
        ) : (
...
function ResultScreen({ level, result, xp, streak, onPlayAnother, onHome, onBackToChapter, onNextGame }) {
  return (
    <GameResults
      ...
      playAgainLabel="மேலும் பழமொழிகள்"
      onDashboard={onHome}
...
function ProverbMatchGame() {
  ...
  if (stage === "loading") return <GameLoadingState label="பழமொழி பொருத்தம் ஏற்றப்படுகிறது..." />;
  if (stage === "error")
    return (
