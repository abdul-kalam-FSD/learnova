            ...
            >
              <span className="flex flex-col">
                ...
                <span className="hint-text text-xs">
                  {level.payload.scrambled_words.length}-word sentence
                </span>
              </span>
    ...
    <GamePage>
      ...
      <GamePanel>
        ...
        <p className="hint-text text-xs mb-2">
          Your sentence so far (tap a word to undo back to it):
        </p>
        <div className="flex flex-wrap gap-2 mb-5 min-h-[3rem] items-center">
          {placed.length === 0 && <span className="hint-text text-sm">— tap words below to start —</span>}
          {placed.map((word, i) => (
            <button
              key={word.id}
              onClick={() => undoFrom(i)}
        ...
        <p className="hint-text text-xs mb-2">Tap words to add them in order:</p>
        ...
        {feedback && (
          <GameFeedback
            ...
            verdict={feedback.isCorrect ? "✓ That's a correctly built sentence!" : "✕ That's not quite the right order."}
...
function ResultScreen({ level, result, xp, streak, onPlayAnother, onHome, onBackToChapter, onNextGame }) {
  return (
    <GameResults
      completeLabel="Sentence Complete"
      badgeText="SENTENCE BUILT"
      title={level.title}
      ...
      playAgainLabel="Build Another Sentence"
      onDashboard={onHome}
...
function SentenceBuilderGame() {
  ...
  if (stage === "loading") return <GameLoadingState label="Loading Sentence Builder..." />;
  if (stage === "error")
    return (
