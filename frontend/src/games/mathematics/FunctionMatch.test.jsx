...
import FunctionMatch from "./FunctionMatch";
...
const LEVEL = {
  ...
  difficulty: "medium",
  payload: {
    slots: [{ id: "s1", label: "{(1,2),(2,4),(3,6)}" }],
  },
};

const FULL_PAYLOAD = {
  scenario: "Match each relation to whether it's a function.",
  slots: [{ id: "s1", label: "{(1,2),(2,4),(3,6)}" }],
  components: [
    { id: "c1", label: "Function — each input has exactly one output" },
    { id: "c2", label: "Not a function — an input repeats with different outputs" },
  ],
  hint: "Check whether any input maps to more than one output.",
};
...
describe("FunctionMatch - loading and error", () => {
  test("shows a loading state before content loads", () => {
    api.get.mockImplementation(() => new Promise(() => {}));
    renderGame();
    expect(screen.getByText("Loading Function Match...")).toBeInTheDocument();
  });
...
describe("FunctionMatch - full play flow", () => {
  test("select level -> mission briefing -> pick item, assign to slot -> result screen", async () => {
    ...
    // ---- Play screen ----
    await screen.findByText("Match each relation to whether it's a function.");
    const checkButton = screen.getByRole("button", { name: "Check Matches" });
    expect(checkButton).toBeDisabled();

    fireEvent.click(screen.getByText("Function — each input has exactly one output"));
    fireEvent.click(screen.getByText("{(1,2),(2,4),(3,6)}"));
  ...
  test("an incorrect match shows failure feedback and does not unlock the reward button", async () => {
    ...
    fireEvent.click(screen.getByText("Not a function — an input repeats with different outputs"));
    fireEvent.click(screen.getByText("{(1,2),(2,4),(3,6)}"));
  ...
  test("re-tapping a filled slot with nothing selected frees it back to the tray", async () => {
    ...
    fireEvent.click(screen.getByText("{(1,2),(2,4),(3,6)}"));
    expect(checkButton).toBeDisabled();
    expect(screen.getByText("Function — each input has exactly one output")).toBeInTheDocument();
  });
});
