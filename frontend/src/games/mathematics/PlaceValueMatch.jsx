...
const GAME_TYPE = "MATH_PLACE_VALUE_MATCH";

// ---------- Highlighted-number visual ----------
// Same mapping-equality pattern as ShapeMatch/FractionMatch, but each
// slot renders the full number (Indian place-value grouping, e.g.
// 4,72,981) with one digit highlighted — the student is matching that
// highlighted digit to the place-value name it sits in.
function HighlightedNumber({ number, highlightIndex }) {
  // number is a digit string with no separators, e.g. "472981".
  // highlightIndex counts digits from the left (0 = leftmost digit).
  const digits = number.split("");
  return (
    <span className="font-mono text-lg tracking-wide" aria-label={`Number ${number}`}>
      {digits.map((d, i) => (
        <span
          key={i}
          className={
            i === highlightIndex
              ? "font-extrabold underline decoration-2"
              : "opacity-70"
          }
          style={i === highlightIndex ? { color: "var(--primary-color)" } : undefined}
        >
          {d}
        </span>
      ))}
...
function LevelSelectScreen({ levels, xp, streak, onBack, onPick }) {
  return (
    <GamePage>
      <GameTopBar label="Place Value Match" xp={xp} streak={streak} onBack={onBack} />
      <GamePanel>
        <span className="clue-card__label">MATHEMATICS · LARGE NUMBERS</span>
        <h1 className="text-2xl font-bold mt-1 mb-3">Match the Place Value</h1>
        <p className="hint-text text-sm mb-4">
          Each number has one digit highlighted — match it to the place it
          sits in. Extra cards in the tray are decoys.
        </p>
            ...
            >
              <span className="flex flex-col">
                ...
                <span className="hint-text text-xs">
                  {level.payload.slots.length} numbers
                </span>
              </span>
...
// ---------- Place-value matching play screen ----------
...
function LobbyScreen({ level, levelIndex, totalLevels, xp, streak, xpInfo, onBack, onStart }) {
  return (
    <GamePage>
      ...
      <GameLobby
        title={level.title}
        subjectLabel="MATHEMATICS · LARGE NUMBERS"
...
function MatchScreen({ level, sessionId, xp, streak, onBack, onSolved, levelIndex, totalLevels }) {
  ...
  return (
    <GamePage>
      ...
      />
      <GamePanel>
        <GameObjective>Match each highlighted digit to the place-value name it sits in.</GameObjective>
        ...
        <p className="hint-text text-xs mb-2">
          Numbers (pick a place value below, then tap a number):
        </p>
        <div className="flex flex-col gap-2 mb-5">
          {slots.map((slot) => (
            ...
            >
              <HighlightedNumber number={slot.number} highlightIndex={slot.highlightIndex} />
        ...
        <p className="hint-text text-xs mb-2">Place values:</p>
        ...
        {feedback && (
          <GameFeedback
            ...
            whatYouLearned={
              feedback.isCorrect
                ? "A digit's value depends on its position in the number, not just the digit itself."
                : null
            }
          />
        )}
...
function ResultScreen({ level, result, xp, streak, onPlayAnother, onHome, onBackToChapter, onNextGame }) {
  return (
    <GameResults
      ...
      playAgainLabel="Match More Numbers"
      onDashboard={onHome}
...
function PlaceValueMatchGame() {
  ...
  if (stage === "loading") return <GameLoadingState label="Loading Place Value Match..." />;
  if (stage === "error")
    return (
...
export default PlaceValueMatch;