...
import GeometryBossChallenge from "./GeometryBossChallenge";
...
const LEVEL = {
  id: "lvl1",
  title: "Face the Shape Golem: Round 1",
  difficulty: "medium",
  payload: {
...
function mockHappyPath() {
  api.get.mockImplementation((url) => {
    ...
    if (url === "/home") {
      return Promise.resolve({ data: { streak_count: 0, xp_total: 20 } });
    }
  ...
  api.post.mockImplementation((url) => {
    ...
    if (url === "/games/sess1/complete") {
      return Promise.resolve({
        data: { xpAwarded: 21, newStreak: 1, masteryUpdate: { new_state: "learning" } },
      });
    }
...
describe("GeometryBossChallenge - loading and error", () => {
  test("shows a loading state before content loads", () => {
    api.get.mockImplementation(() => new Promise(() => {}));
    renderGame();
    expect(screen.getByText("Loading Geometry Boss Challenge...")).toBeInTheDocument();
  });
...
describe("GeometryBossChallenge - full play flow", () => {
  test("select round -> land both hits -> golem defeated -> claim reward -> result screen", async () => {
    mockHappyPath();
    renderGame();

    // ---- Round select screen ----
    await screen.findByText("Face the Shape Golem: Round 1");
    expect(screen.getByText("2 questions · 10s each")).toBeInTheDocument();
    fireEvent.click(screen.getByText("Face the Shape Golem: Round 1"));
    ...
    expect(api.post).toHaveBeenCalledWith("/games/start", {
      gameType: "MATH_GEOMETRY_BOSS_CHALLENGE",
      contentId: "lvl1",
    });

    // ---- Attack 1 ----
    await screen.findByText("ATTACK 1 / 2");
    expect(screen.getByText("How many sides does a hexagon have?")).toBeInTheDocument();
    fireEvent.click(screen.getByText("6"));

    // ---- Advances to Attack 2 after the feedback pause ----
    await waitFor(() => expect(screen.getByText("ATTACK 2 / 2")).toBeInTheDocument());
    expect(screen.getByText("What is the sum of angles in a triangle?")).toBeInTheDocument();
    ...
    expect(screen.getByText("✓ The Shape Golem is down!")).toBeInTheDocument();
    ...
    // ---- Result screen ----
    await screen.findByText("+21 XP");
    expect(screen.getByText("Updated to: learning")).toBeInTheDocument();
    expect(screen.getByRole("button", { name: "Rematch the Golem" })).toBeInTheDocument();
  ...
  test("a single miss leaves the golem standing", async () => {
    mockHappyPath();
    api.post.mockImplementation((url) => {
      if (url === "/games/start") {
      ...
      if (url === "/games/sess1/attempt") {
        return Promise.resolve({
          data: {
            ...
            totalCount: 2,
            isCorrect: false,
            hint: "Recount the sides carefully.",
          },
        });
      }
      return Promise.reject(new Error(`unmocked POST ${url}`));
    ...
    expect(
      screen.getByText("The Golem is still standing — one miss was all it took."),
    ).toBeInTheDocument();
    expect(screen.getByText("Recount the sides carefully.")).toBeInTheDocument();
  });
  ...
  test("letting the timer run out submits a null answer and shows the timeout message", async () => {
    ...
    await waitFor(
      () => expect(screen.getByText("⏱ Time's up — the Golem shrugs it off.")).toBeInTheDocument(),
      { timeout: 3000 },
    );
