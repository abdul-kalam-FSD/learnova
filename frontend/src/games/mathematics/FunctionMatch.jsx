...
const GAME_TYPE = "MATH_FUNCTION_MATCH";

// ---------- Level select ----------
function LevelSelectScreen({ levels, xp, streak, onBack, onPick }) {
  return (
    <GamePage>
      <GameTopBar label="Function Match" xp={xp} streak={streak} onBack={onBack} />
      <GamePanel>
        <span className="clue-card__label">MATHEMATICS · RELATIONS AND FUNCTIONS</span>
        <h1 className="text-2xl font-bold mt-1 mb-3">Match the Relations</h1>
        <p className="hint-text text-sm mb-4">
          Match each relation to its correct domain/range or function-type
          classification — extra cards in the tray are decoys, so
            ...
            >
              <span className="flex flex-col">
                ...
                <span className="hint-text text-xs">
                  {level.payload.slots.length} relations
                </span>
              </span>
...
function LobbyScreen({ level, levelIndex, totalLevels, xp, streak, xpInfo, onBack, onStart }) {
  return (
    <GamePage>
      ...
      <GameLobby
        title={level.title}
        subjectLabel="MATHEMATICS · RELATIONS AND FUNCTIONS"
...
function MatchScreen({ level, sessionId, xp, streak, onBack, onSolved, levelIndex, totalLevels }) {
  ...
  return (
    <GamePage>
      ...
      />
      <GamePanel>
        <GameObjective>Match each relation or function to its correct property or output.</GameObjective>
        ...
        <p className="hint-text text-xs mb-2">
          Relations (pick a classification below, then tap a card):
        </p>
        ...
        <p className="hint-text text-xs mb-2">Classifications:</p>
        ...
        {feedback && (
          <GameFeedback
            ...
            whatYouLearned={
              feedback.isCorrect
                ? "A function assigns exactly one output to each input — that's what separates functions from general relations."
                : null
            }
          />
        )}
...
function ResultScreen({ level, result, xp, streak, onPlayAnother, onHome, onBackToChapter, onNextGame }) {
  return (
    <GameResults
      ...
      playAgainLabel="Match More Relations"
      onDashboard={onHome}
...
function FunctionMatchGame() {
  ...
  if (stage === "loading") return <GameLoadingState label="Loading Function Match..." />;
  if (stage === "error")
    return (
...
// FunctionMatch is migrated to the focused game-session mode (Phase 5C-A)
