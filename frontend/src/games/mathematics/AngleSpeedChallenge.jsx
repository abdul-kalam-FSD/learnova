...
const GAME_TYPE = "MATH_ANGLE_SPEED_CHALLENGE";
...
function RoundSelectScreen({ levels, xp, streak, onBack, onPick }) {
  return (
    <GamePage>
      <GameTopBar label="Angle Speed Challenge" xp={xp} streak={streak} onBack={onBack} />
      <GamePanel>
...
function RoundScreen({ level, xp, streak, onBack, onRoundScored, levelIndex, totalLevels }) {
  ...
  return (
    <GamePage>
      ...
      />
      <GamePanel>
        <GameObjective>Identify each angle's measure or relationship correctly before the timer runs out.</GameObjective>
    ...
    <GamePage>
      ...
      <GamePanel>
        ...
        <GameFeedback
          ...
          whatYouLearned={
            isCorrect
              ? "Recognizing common angle relationships (complementary, supplementary, vertical) instantly speeds up geometry problem solving."
              : null
          }
        />
...
function AngleSpeedChallengeGame() {
  ...
  if (stage === "loading") return <GameLoadingState label="Loading Angle Speed Challenge..." />;
  if (stage === "error")
    return (
...
// AngleSpeedChallenge is migrated to the focused game-session mode (Phase 5C-A)
// -- wrapped in GameFrame so it renders like the six Phase 1B/1C
// representative games already did (see GameFrame.jsx).
function AngleSpeedChallenge() {
  return (
    <GameFrame>
      <AngleSpeedChallengeGame />
    </GameFrame>
  );
}

export default AngleSpeedChallenge;
