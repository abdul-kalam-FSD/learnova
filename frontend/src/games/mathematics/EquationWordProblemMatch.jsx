...
const GAME_TYPE = "MATH_EQUATION_WORD_PROBLEM_MATCH";

// ---------- Level select ----------
function LevelSelectScreen({ levels, xp, streak, onBack, onPick }) {
  return (
    <GamePage>
      <GameTopBar label="Equation Word Problem Match" xp={xp} streak={streak} onBack={onBack} />
      <GamePanel>
        <span className="clue-card__label">MATHEMATICS · SIMPLE EQUATIONS</span>
        <h1 className="text-2xl font-bold mt-1 mb-3">Match the Word Problems</h1>
        <p className="hint-text text-sm mb-4">
          Match each real-world story to the equation that represents it —
          extra equations in the tray are decoys, so read carefully before
          you assign.
        </p>
            ...
            >
              <span className="flex flex-col">
                ...
                <span className="hint-text text-xs">
                  {level.payload.slots.length} stories
                </span>
              </span>
...
// ---------- Word problem-matching play screen ----------
function MatchScreen({ level, sessionId, xp, streak, onBack, onSolved, levelIndex, totalLevels }) {
  ...
  return (
    <GamePage>
      ...
      />
      <GamePanel>
        <GameObjective>Match each real-world story to the equation that represents it.</GameObjective>
        ...
        <p className="hint-text text-xs mb-2">
          Stories (pick the matching equation below, then tap a story):
        </p>
        ...
        <p className="hint-text text-xs mb-2">Equations:</p>
        ...
        {feedback && (
          <GameFeedback
            ...
            whatYouLearned={
              feedback.isCorrect
                ? "Translating a word problem into an equation is the key step — once it's an equation, you can solve it with standard rules."
                : null
            }
          />
        )}
...
function ResultScreen({ level, result, xp, streak, onPlayAnother, onHome, onBackToChapter, onNextGame }) {
  return (
    <GameResults
      ...
      badgeText="WORD PROBLEM MATCH COMPLETE"
      title={level.title}
      ...
      playAgainLabel="Match More Stories"
      onDashboard={onHome}
...
function EquationWordProblemMatchGame() {
  ...
  if (stage === "loading") return <GameLoadingState label="Loading Equation Word Problem Match..." />;
  if (stage === "error")
    return (
...
// EquationWordProblemMatch is migrated to the focused game-session mode (Phase 5C-A)
// -- wrapped in GameFrame so it renders like the six Phase 1B/1C
// representative games already did (see GameFrame.jsx).
function EquationWordProblemMatch() {
  return (
    <GameFrame>
      <EquationWordProblemMatchGame />
    </GameFrame>
  );
}

export default EquationWordProblemMatch;
