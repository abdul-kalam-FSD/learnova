...
const GAME_TYPE = "MATH_TRIG_MATCH";

// ---------- Level select ----------
function LevelSelectScreen({ levels, xp, streak, onBack, onPick }) {
  return (
    <GamePage>
      <GameTopBar label="Trig Match" xp={xp} streak={streak} onBack={onBack} />
      <GamePanel>
        <span className="clue-card__label">MATHEMATICS · TRIGONOMETRIC FUNCTIONS</span>
        <h1 className="text-2xl font-bold mt-1 mb-3">Match the Angles</h1>
        <p className="hint-text text-sm mb-4">
          Match each angle to its correct trig value or radian
          equivalent — extra cards in the tray are decoys, so
            ...
            >
              <span className="flex flex-col">
                ...
                <span className="hint-text text-xs">
                  {level.payload.slots.length} angles
                </span>
              </span>
...
function LobbyScreen({ level, levelIndex, totalLevels, xp, streak, xpInfo, onBack, onStart }) {
  return (
    <GamePage>
      ...
      <GameLobby
        title={level.title}
        subjectLabel="MATHEMATICS · TRIGONOMETRIC FUNCTIONS"
...
function MatchScreen({ level, sessionId, xp, streak, onBack, onSolved, levelIndex, totalLevels }) {
  ...
  return (
    <GamePage>
      ...
      />
      <GamePanel>
        <GameObjective>Match each angle or trig expression to its correct value.</GameObjective>
        ...
        <p className="hint-text text-xs mb-2">
          Angles (pick a value below, then tap a card):
        </p>
        ...
        <p className="hint-text text-xs mb-2">Values:</p>
        ...
        {feedback && (
          <GameFeedback
            ...
            whatYouLearned={
              feedback.isCorrect
                ? "Standard angles have exact trigonometric ratios that repeat in a predictable pattern."
                : null
            }
          />
        )}
...
function ResultScreen({ level, result, xp, streak, onPlayAnother, onHome, onBackToChapter, onNextGame }) {
  return (
    <GameResults
      ...
      playAgainLabel="Match More Angles"
      onDashboard={onHome}
...
function TrigMatchGame() {
  ...
  if (stage === "loading") return <GameLoadingState label="Loading Trig Match..." />;
  if (stage === "error")
    return (
...
// TrigMatch is migrated to the focused game-session mode (Phase 5C-A)
