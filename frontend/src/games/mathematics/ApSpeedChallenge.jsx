...
const GAME_TYPE = "MATH_AP_SPEED_CHALLENGE";
...
function RoundSelectScreen({ levels, xp, streak, onBack, onPick }) {
  return (
    <GamePage>
      <GameTopBar label="Arithmetic Progression Speed Challenge" xp={xp} streak={streak} onBack={onBack} />
      <GamePanel>
        <span className="clue-card__label">MATHEMATICS · SEQUENCES AND SERIES</span>
        <h1 className="text-2xl font-bold mt-1 mb-3">Beat the Clock</h1>
        <p className="hint-text text-sm mb-4">
          Quick-fire nth-term and sum-of-n-terms calculations — answer every
          question before time runs out, no time to double-check.
        </p>
...
function LobbyScreen({ level, levelIndex, totalLevels, xp, streak, xpInfo, onBack, onStart }) {
  ...
  return (
    <GamePage>
      ...
      <GameLobby
        title={level.title}
        subjectLabel="MATHEMATICS · SEQUENCES AND SERIES"
        objective={
...
function RoundScreen({ level, xp, streak, onBack, onRoundScored, levelIndex, totalLevels }) {
  ...
  return (
    <GamePage>
      ...
      />
      <GamePanel>
        <GameObjective>Find each term or sum in the arithmetic progression correctly before the timer runs out.</GameObjective>
    ...
    <GamePage>
      ...
      <GamePanel>
        ...
        <GameFeedback
          ...
          whatYouLearned={
            isCorrect
              ? "An arithmetic progression grows by a fixed common difference, which is the key to finding any term quickly."
              : null
          }
        />
...
function ApSpeedChallengeGame() {
  ...
  if (stage === "loading") return <GameLoadingState label="Loading AP Speed Challenge..." />;
  if (stage === "error")
    return (
...
// ApSpeedChallenge is migrated to the focused game-session mode (Phase 5C-A)
// -- wrapped in GameFrame so it renders like the six Phase 1B/1C
// representative games already did (see GameFrame.jsx).
function ApSpeedChallenge() {
  return (
    <GameFrame>
      <ApSpeedChallengeGame />
    </GameFrame>
  );
}

export default ApSpeedChallenge;
