...
const GAME_TYPE = "MATH_LINE_EQUATION_MATCH";

// ---------- Level select ----------
function LevelSelectScreen({ levels, xp, streak, onBack, onPick }) {
  return (
    <GamePage>
      <GameTopBar label="Line Equation Match" xp={xp} streak={streak} onBack={onBack} />
      <GamePanel>
        <span className="clue-card__label">MATHEMATICS · STRAIGHT LINES</span>
        <h1 className="text-2xl font-bold mt-1 mb-3">Match the Lines</h1>
        <p className="hint-text text-sm mb-4">
          Match each line description — two points, or a point and a
          slope — to its correct equation. Extra cards in the tray
            ...
            >
              <span className="flex flex-col">
                ...
                <span className="hint-text text-xs">
                  {level.payload.slots.length} lines
                </span>
              </span>
...
function LobbyScreen({ level, levelIndex, totalLevels, xp, streak, xpInfo, onBack, onStart }) {
  return (
    <GamePage>
      ...
      <GameLobby
        title={level.title}
        subjectLabel="MATHEMATICS · STRAIGHT LINES"
...
function MatchScreen({ level, sessionId, xp, streak, onBack, onSolved, levelIndex, totalLevels }) {
  ...
  return (
    <GamePage>
      ...
      />
      <GamePanel>
        <GameObjective>Match each straight-line equation to its correct slope, intercept, or graph description.</GameObjective>
        ...
        <p className="hint-text text-xs mb-2">
          Line descriptions (pick an equation below, then tap a card):
        </p>
        ...
        {feedback && (
          <GameFeedback
            ...
            whatYouLearned={
              feedback.isCorrect
                ? "A line's equation encodes both its slope and where it crosses the axes."
                : null
            }
          />
        )}
...
function ResultScreen({ level, result, xp, streak, onPlayAnother, onHome, onBackToChapter, onNextGame }) {
  return (
    <GameResults
      ...
      playAgainLabel="Match More Lines"
      onDashboard={onHome}
...
function LineEquationMatchGame() {
  ...
  if (stage === "loading") return <GameLoadingState label="Loading Line Equation Match..." />;
  if (stage === "error")
    return (
...
// LineEquationMatch is migrated to the focused game-session mode (Phase 5C-A)
// -- wrapped in GameFrame so it renders like the six Phase 1B/1C
// representative games already did (see GameFrame.jsx).
function LineEquationMatch() {
  return (
    <GameFrame>
      <LineEquationMatchGame />
    </GameFrame>
  );
}

export default LineEquationMatch;