...
const GAME_TYPE = "MATH_FRACTION_SPEED_CHALLENGE";
const IDENTITY = "speed";
// Long enough to see the correct/wrong flash before the next question
// loads, short enough that a quick-fire round still feels quick.
...
function RoundSelectScreen({ levels, xp, streak, onBack, onPick }) {
  return (
    <GamePage identity={IDENTITY}>
      <GameTopBar label="Fraction Speed Challenge" xp={xp} streak={streak} onBack={onBack} />
      <GamePanel>
        <span className="clue-card__label game-identity-badge">
          <GameIdentityMark identity={IDENTITY} />
          MATHEMATICS · FRACTIONS
        </span>
        <h1 className="text-2xl font-bold mt-1 mb-3">Beat the Clock</h1>
        <p className="hint-text text-sm mb-4">
          Answer every question before time runs out — no time to double-check,
...
// Round-loop state (index/timeLeft/flash/advance-on-answer-or-timeout)
// now lives in useRoundTimer (Phase 1B §6-8) — extracted verbatim from
// this component, which was the literal template it was copied from.
// Only the option-tapping UI below (mechanic-specific, per Phase 1A
// §9) stays local.
function RoundScreen({ level, xp, streak, onBack, onRoundScored, levelIndex, totalLevels }) {
  ...
  // The client never receives correct_option_id (sanitized server-side —
  // see gameControllers.sanitizePayloadForClient), so per-option
  // correct/wrong color isn't possible until the whole round is
  // scored. This flash only confirms the tap registered; the real
  // score shows on RoundResultScreen once the batch attempt returns.
  ...
  return (
    <GamePage identity={IDENTITY}>
      ...
      />
      <GamePanel>
        <GameObjective>Simplify or compare fractions correctly before the timer runs out.</GameObjective>
        ...
        {flash?.isTimeout && (
          <p className="hint-text text-xs">⏱ Time's up — moving on.</p>
        )}
      </GamePanel>
    </GamePage>
  );
    ...
    <GamePage identity={IDENTITY}>
      ...
      <GamePanel>
        ...
        <GameFeedback
          ...
          whatYouLearned={
            isCorrect
              ? "Speed comes from recognizing common fraction patterns instantly, not from calculating them from scratch every time."
              : null
          }
        />
...
function ResultScreen({ level, result, scoreResult, xp, streak, onPlayAnother, onHome, onBackToChapter, onNextGame }) {
  return (
    <GameResults
      ...
      badgeText="SPEED CHALLENGE COMPLETE"
      title={level.title}
...
function FractionSpeedChallengeGame() {
  ...
  // Called once the round's last question is answered/timed out —
  // submits the whole batch of answers as a single attempt.
  ...
  if (stage === "loading") return <GameLoadingState label="Loading Fraction Speed Challenge..." />;
  if (stage === "error")
    return (
...
// FractionSpeedChallenge is one of the six Phase 1B representative
// games (§15) — wrapped in GameFrame so it renders in the focused
// game-session mode (global header/bottom-nav suppressed) across every
// stage (select/lobby/play/result), while every stage above stays
// exactly as it was.
function FractionSpeedChallenge() {
  return (
    <GameFrame>
      <FractionSpeedChallengeGame />
    </GameFrame>
  );
}

export default FractionSpeedChallenge;