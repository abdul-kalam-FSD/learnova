...
const GAME_TYPE = "MATH_EQUATION_BOSS_CHALLENGE";
...
function RoundSelectScreen({ levels, xp, streak, onBack, onPick }) {
  return (
    <GamePage>
      <GameTopBar label="Equation Boss Challenge" xp={xp} streak={streak} onBack={onBack} />
      <GamePanel>
        <span className="clue-card__label">MATHEMATICS · EQUATIONS · BOSS</span>
        <h1 className="text-2xl font-bold mt-1 mb-3">Face the Balance Keeper</h1>
        <p className="hint-text text-sm mb-4">
          The Balance Keeper only falls to a clean sweep — solving for x,
          quick calculation and balance-strategy reasoning all mixed into one
          fast round. Every wrong answer lets it stand a little longer.
        </p>
...
function LobbyScreen({ level, levelIndex, totalLevels, xp, streak, xpInfo, onBack, onStart }) {
  ...
  return (
    <GamePage>
      ...
      <GameLobby
        title={level.title}
        subjectLabel="MATHEMATICS · EQUATIONS · BOSS"
        objective={
...
function RoundScreen({ level, xp, streak, onBack, onRoundScored, levelIndex, totalLevels }) {
  ...
  return (
    <GamePage>
      ...
      />
      <GamePanel>
        <GameObjective>Solve each equation correctly before the timer runs out to land a hit on the boss.</GameObjective>
        ...
        {flash?.isTimeout && (
          <p className="hint-text text-xs">⏱ Time's up — the Balance Keeper shrugs it off.</p>
        )}
      </GamePanel>
    </GamePage>
  );
  ...
  return (
    <GamePage>
      ...
      <GamePanel>
        ...
        <GameFeedback
          isCorrect={isCorrect}
          verdict={isCorrect ? "✓ The Balance Keeper is down!" : "The Balance Keeper is still standing — one miss was all it took."}
          explanation={!isCorrect ? hint : null}
          whatYouLearned={
            isCorrect
              ? "Solving for x means isolating the variable using inverse operations, applied consistently to both sides."
              : null
          }
        />
...
function ResultScreen({ level, result, scoreResult, xp, streak, onPlayAnother, onHome, onBackToChapter, onNextGame }) {
  return (
    <GameResults
      ...
      playAgainLabel="Rematch the Keeper"
      onDashboard={onHome}
...
function EquationBossChallengeGame() {
  ...
  if (stage === "loading") return <GameLoadingState label="Loading Equation Boss Challenge..." />;
  if (stage === "error")
    return (
...
// EquationBossChallenge is migrated to the focused game-session mode (Phase 5C-A)
// -- wrapped in GameFrame so it renders like the six Phase 1B/1C
// representative games already did (see GameFrame.jsx).
function EquationBossChallenge() {
  return (
    <GameFrame>
      <EquationBossChallengeGame />
    </GameFrame>
  );
}

export default EquationBossChallenge;
