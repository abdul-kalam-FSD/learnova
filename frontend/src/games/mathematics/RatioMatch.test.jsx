...
import RatioMatch from "./RatioMatch";
...
const LEVEL = {
  ...
  difficulty: "medium",
  payload: {
    slots: [{ id: "s1", label: "8 : 12" }],
  },
};

const FULL_PAYLOAD = {
  scenario: "Match each ratio to its simplest form.",
  slots: [{ id: "s1", label: "8 : 12" }],
  components: [
    { id: "c1", label: "2 : 3" },
    { id: "c2", label: "4 : 6" },
...
describe("RatioMatch - loading and error", () => {
  test("shows a loading state before content loads", () => {
    api.get.mockImplementation(() => new Promise(() => {}));
    renderGame();
    expect(screen.getByText("Loading Ratio Match...")).toBeInTheDocument();
  });
...
describe("RatioMatch - full play flow", () => {
  test("select level -> mission briefing -> pick item, assign to slot -> result screen", async () => {
    ...
    // ---- Play screen ----
    await screen.findByText("Match each ratio to its simplest form.");
