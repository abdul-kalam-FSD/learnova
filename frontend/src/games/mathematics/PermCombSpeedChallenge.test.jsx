...
import PermCombSpeedChallenge from "./PermCombSpeedChallenge";
...
const LEVEL = {
  id: "lvl1",
  title: "Speed Round: Permutations vs Combinations",
  difficulty: "medium",
  payload: {
...
const FULL_PAYLOAD = {
  ...
  questions: [
    {
      id: "q1",
      prompt: "Choosing 2 books from 4 to read in order is a:",
      options: [
        { id: "a", label: "Permutation" },
        { id: "b", label: "Combination" },
      ],
    },
    {
      id: "q2",
      prompt: "Choosing 2 books from 4 to take on a trip (order doesn't matter) is a:",
      options: [
        { id: "a", label: "Permutation" },
        { id: "b", label: "Combination" },
      ],
...
describe("PermCombSpeedChallenge - loading and error", () => {
  test("shows a loading state before content loads", () => {
    api.get.mockImplementation(() => new Promise(() => {}));
    renderGame();
    expect(screen.getByText("Loading Permutations & Combinations Speed Challenge...")).toBeInTheDocument();
  });
...
describe("PermCombSpeedChallenge - full play flow", () => {
  test("select round -> tap an answer for both questions -> perfect round -> claim reward -> result screen", async () => {
    mockHappyPath();
    renderGame();

    // ---- Round select screen ----
    await screen.findByText("Speed Round: Permutations vs Combinations");
    expect(screen.getByText("2 questions · 10s each")).toBeInTheDocument();
    fireEvent.click(screen.getByText("Speed Round: Permutations vs Combinations"));
    ...
    expect(api.post).toHaveBeenCalledWith("/games/start", {
      gameType: "MATH_PERMCOMB_SPEED_CHALLENGE",
      contentId: "lvl1",
    });

    // ---- Question 1 ----
    await screen.findByText("QUESTION 1 / 2");
    expect(screen.getByText("Choosing 2 books from 4 to read in order is a:")).toBeInTheDocument();
    fireEvent.click(screen.getByText("Permutation"));

    // ---- Advances to Question 2 after the feedback pause ----
    await waitFor(() => expect(screen.getByText("QUESTION 2 / 2")).toBeInTheDocument());
    expect(screen.getByText("Choosing 2 books from 4 to take on a trip (order doesn't matter) is a:")).toBeInTheDocument();

    fireEvent.click(screen.getByText("Combination"));
  ...
  test("an imperfect round shows the 'nice try' verdict and explanation", async () => {
      ...
      if (url === "/games/sess1/attempt") {
        return Promise.resolve({
          data: {
            ...
            totalCount: 2,
            isCorrect: false,
            hint: "If order matters, it's a permutation; if it doesn't, it's a combination.",
          },
        });
      }
      return Promise.reject(new Error(`unmocked POST ${url}`));
    ...
    expect(screen.getByText("If order matters, it's a permutation; if it doesn't, it's a combination.")).toBeInTheDocument();
  });
