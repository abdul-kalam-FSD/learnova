...
const GAME_TYPE = "MATH_PERMCOMB_SPEED_CHALLENGE";
...
function RoundSelectScreen({ levels, xp, streak, onBack, onPick }) {
  return (
    <GamePage>
      <GameTopBar label="Permutations & Combinations Speed Challenge" xp={xp} streak={streak} onBack={onBack} />
      <GamePanel>
        <span className="clue-card__label">MATHEMATICS · PERMUTATIONS & COMBINATIONS</span>
        <h1 className="text-2xl font-bold mt-1 mb-3">Beat the Clock</h1>
        <p className="hint-text text-sm mb-4">
          Quick-fire nPr and nCr calculations — answer every question before
          time runs out, no time to double-check.
        </p>
...
function LobbyScreen({ level, levelIndex, totalLevels, xp, streak, xpInfo, onBack, onStart }) {
  ...
  return (
    <GamePage>
      ...
      <GameLobby
        title={level.title}
        subjectLabel="MATHEMATICS · PERMUTATIONS & COMBINATIONS"
        objective={
...
function RoundScreen({ level, xp, streak, onBack, onRoundScored, levelIndex, totalLevels }) {
  ...
  return (
    <GamePage>
      ...
      />
      <GamePanel>
        <GameObjective>Work out each permutation or combination count correctly before the timer runs out.</GameObjective>
    ...
    <GamePage>
      ...
      <GamePanel>
        ...
        <GameFeedback
          ...
          whatYouLearned={
            isCorrect
              ? "Knowing whether order matters is the fastest way to decide between a permutation and a combination."
              : null
          }
        />
...
function PermCombSpeedChallengeGame() {
  ...
  if (stage === "loading") return <GameLoadingState label="Loading Permutations & Combinations Speed Challenge..." />;
  if (stage === "error")
    return (
...
// PermCombSpeedChallenge is migrated to the focused game-session mode (Phase 5C-A)
// -- wrapped in GameFrame so it renders like the six Phase 1B/1C
// representative games already did (see GameFrame.jsx).
function PermCombSpeedChallenge() {
  return (
    <GameFrame>
      <PermCombSpeedChallengeGame />
    </GameFrame>
  );
}

export default PermCombSpeedChallenge;
