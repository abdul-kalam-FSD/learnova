...
import EquationSpeedCalculation from "./EquationSpeedCalculation";
...
const FULL_PAYLOAD = {
  time_limit_seconds: 10,
  questions: [
    { id: "q1", equation_label: "Solve for x: 3x + 5 = 20" },
    { id: "q2", equation_label: "Solve for x: 2x - 4 = 10" },
  ],
};
...
describe("EquationSpeedCalculation - loading and error", () => {
  test("shows a loading state before content loads", () => {
    api.get.mockImplementation(() => new Promise(() => {}));
    renderGame();
    expect(screen.getByText("Loading Equation Speed Calculation...")).toBeInTheDocument();
  });
...
describe("EquationSpeedCalculation - full play flow", () => {
  test("select round -> answer both questions -> perfect round -> claim reward -> result screen", async () => {
    mockHappyPath();
    renderGame();

    // ---- Round select screen ----
    await screen.findByText("Speed Round: Solve for x");
    expect(screen.getByText("2 questions · 10s each")).toBeInTheDocument();
    fireEvent.click(screen.getByText("Speed Round: Solve for x"));
    ...
    expect(api.post).toHaveBeenCalledWith("/games/start", {
      gameType: "MATH_EQUATION_SPEED_CALCULATION",
      contentId: "lvl1",
    });

    // ---- Question 1 ----
    await screen.findByText("QUESTION 1 / 2");
    expect(screen.getByText("Solve for x: 3x + 5 = 20")).toBeInTheDocument();
    ...
    fireEvent.change(screen.getByPlaceholderText("?"), { target: { value: "7" } });
  ...
  test("an imperfect round shows the 'nice try' verdict and explanation", async () => {
      ...
      if (url === "/games/sess1/attempt") {
        return Promise.resolve({
          data: {
            ...
            totalCount: 2,
            isCorrect: false,
            hint: "Isolate x by undoing addition or subtraction first, then multiplication or division.",
          },
        });
      }
      return Promise.reject(new Error(`unmocked POST ${url}`));
    ...
    expect(screen.getByText("Isolate x by undoing addition or subtraction first, then multiplication or division.")).toBeInTheDocument();
  });
