...
const GAME_TYPE = "MATH_INEQUALITY_MATCH";

// ---------- Level select ----------
function LevelSelectScreen({ levels, xp, streak, onBack, onPick }) {
  return (
    <GamePage>
      <GameTopBar label="Inequality Match" xp={xp} streak={streak} onBack={onBack} />
      <GamePanel>
        <span className="clue-card__label">MATHEMATICS · LINEAR INEQUALITIES</span>
        <h1 className="text-2xl font-bold mt-1 mb-3">Match the Inequalities</h1>
        <p className="hint-text text-sm mb-4">
          Match each linear inequality to its correct solution
          interval — extra cards in the tray are decoys, so
          double-check before you assign.
        </p>
            ...
            >
              <span className="flex flex-col">
                ...
                <span className="hint-text text-xs">
                  {level.payload.slots.length} inequalities
                </span>
              </span>
...
// ---------- Inequality-matching play screen ----------
...
function LobbyScreen({ level, levelIndex, totalLevels, xp, streak, xpInfo, onBack, onStart }) {
  return (
    <GamePage>
      ...
      <GameLobby
        title={level.title}
        subjectLabel="MATHEMATICS · LINEAR INEQUALITIES"
...
function MatchScreen({ level, sessionId, xp, streak, onBack, onSolved, levelIndex, totalLevels }) {
  ...
  return (
    <GamePage>
      ...
      />
      <GamePanel>
        <GameObjective>Match each linear inequality to its correct solution interval.</GameObjective>
        ...
        <p className="hint-text text-xs mb-2">
          Inequalities (pick a solution below, then tap a card):
        </p>
        ...
        <p className="hint-text text-xs mb-2">Solutions:</p>
        ...
        {feedback && (
          <GameFeedback
            ...
            whatYouLearned={
              feedback.isCorrect
                ? "The solution set of a linear inequality is an interval or ray on the number line, not a single point."
                : null
            }
          />
        )}
...
function ResultScreen({ level, result, xp, streak, onPlayAnother, onHome, onBackToChapter, onNextGame }) {
  return (
    <GameResults
      ...
      playAgainLabel="Match More Inequalities"
      onDashboard={onHome}
...
function InequalityMatchGame() {
  ...
  if (stage === "loading") return <GameLoadingState label="Loading Inequality Match..." />;
  if (stage === "error")
    return (
...
export default InequalityMatch;
