...
const GAME_TYPE = "MATH_RATIO_MATCH";

// ---------- Level select ----------
function LevelSelectScreen({ levels, xp, streak, onBack, onPick }) {
  return (
    <GamePage>
      <GameTopBar label="Ratio Match" xp={xp} streak={streak} onBack={onBack} />
      <GamePanel>
        <span className="clue-card__label">MATHEMATICS · RATIO AND PROPORTION</span>
        <h1 className="text-2xl font-bold mt-1 mb-3">Match the Ratios</h1>
        <p className="hint-text text-sm mb-4">
          Match each ratio card to its correct equivalent or simplest form — extra cards in
            ...
            >
              <span className="flex flex-col">
                ...
                <span className="hint-text text-xs">
                  {level.payload.slots.length} ratios
                </span>
              </span>
...
function LobbyScreen({ level, levelIndex, totalLevels, xp, streak, xpInfo, onBack, onStart }) {
  return (
    <GamePage>
      ...
      <GameLobby
        title={level.title}
        subjectLabel="MATHEMATICS · RATIO AND PROPORTION"
...
function MatchScreen({ level, sessionId, xp, streak, onBack, onSolved, levelIndex, totalLevels }) {
  ...
  return (
    <GamePage>
      ...
      />
      <GamePanel>
        <GameObjective>Match each ratio card to its correct equivalent or simplest form.</GameObjective>
        ...
        <p className="hint-text text-xs mb-2">
          Ratio cards (pick an equivalent below, then tap a card):
        </p>
        ...
        {feedback && (
          <GameFeedback
            ...
            whatYouLearned={
              feedback.isCorrect
                ? "Equivalent ratios describe the same proportion even when their terms differ."
                : null
            }
          />
        )}
...
function ResultScreen({ level, result, xp, streak, onPlayAnother, onHome, onBackToChapter, onNextGame }) {
  return (
    <GameResults
      ...
      playAgainLabel="Match More Ratios"
      onDashboard={onHome}
...
function RatioMatchGame() {
  ...
  if (stage === "loading") return <GameLoadingState label="Loading Ratio Match..." />;
  if (stage === "error")
    return (
...
// RatioMatch is migrated to the focused game-session mode (Phase 5C-A)
