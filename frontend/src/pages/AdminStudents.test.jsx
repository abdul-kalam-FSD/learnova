...
const STUDENTS = [
  { id: "s1", name: "Priya Kumar", grade: 8, email: "priya@example.com", xpTotal: 340 },
  { id: "s2", name: "Arjun Rao", grade: 8, email: "arjun@example.com", xpTotal: 210 },
];
...
describe("AdminStudents", () => {
  ...
  test("renders the student list and total count", async () => {
  ...
  test("clicking a student row navigates to their detail page", async () => {
    api.get.mockResolvedValue({ data: page(STUDENTS) });
    renderPage();
    ...
    expect(navigateMock).toHaveBeenCalledWith("/admin/students/s1");
  });
  ...
  test("typing in search debounces before triggering a request", async () => {
    ...
    // Not yet — still well within the 350ms debounce window.
