...
async function seed() {
  ...
  if (!subject) {
    console.error("Grade 10 Science/Biology subject not found — run seedGrade10.js first.");
  ...
  const conceptChapters = {
    ...
    "Asexual Reproduction": "How Do Organisms Reproduce",
  ...
  for (const [title, chapterTitle] of Object.entries(conceptChapters)) {
    ...
    if (!chapter) {
      console.error(`Chapter "${chapterTitle}" not found — run seedGrade10.js first.`);
      await mongoose.disconnect();
      process.exit(1);
    }
