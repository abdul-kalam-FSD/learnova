...
async function seed() {
  ...
  const chapter = await Chapter.findOne({ subject_id: subject._id, title: "Animal Kingdom" });
  if (!chapter) {
    console.error("Chapter 'Animal Kingdom' not found.");
    await mongoose.disconnect();
    process.exit(1);
  }

  const concept = await Concept.findOne({ chapter_id: chapter._id, title: "Basis of Classification" });
  if (!concept) {
    console.error(
