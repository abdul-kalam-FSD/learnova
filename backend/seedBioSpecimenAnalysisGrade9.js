...
async function seed() {
  ...
  const chapter = await Chapter.findOne({ subject_id: subject._id, title: "Diversity in Living Organisms" });
  if (!chapter) {
    console.error("Chapter 'Diversity in Living Organisms' not found.");
    await mongoose.disconnect();
    process.exit(1);
  }

  const concept = await Concept.findOne({ chapter_id: chapter._id, title: "Vertebrates and Invertebrates" });
  if (!concept) {
    console.error(
