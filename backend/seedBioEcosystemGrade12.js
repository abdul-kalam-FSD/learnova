...
async function seed() {
  ...
  const chapter = await Chapter.findOne({ subject_id: subject._id, title: "Ecosystem" });
  if (!chapter) {
    console.error("Chapter 'Ecosystem' not found.");
    await mongoose.disconnect();
    process.exit(1);
  }

  const concept = await Concept.findOne({ chapter_id: chapter._id, title: "Decomposition and Energy Flow" });
  if (!concept) {
    console.error(
